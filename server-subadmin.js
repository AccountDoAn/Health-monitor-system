...
<!-- MODAL: SỬA LIÊN KẾT NGƯỜI NHÀ (từ tab bệnh nhân) -->
<div class="modal-bg" id="modal-edit-pt-family" onclick="if(event.target===this)closeModal('modal-edit-pt-family')">
  <div class="modal modal-sm">
    <button class="modal-x" onclick="closeModal('modal-edit-pt-family')"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="18" y1="6" x2="6" y2="18"/><line x1="6" y1="6" x2="18" y2="18"/></svg></button>
    <div class="modal-ttl">✏️ Sửa liên kết người nhà</div>
    <input type="hidden" id="epf-old-link-id"/>
    <input type="hidden" id="epf-patient-id"/>
    <input type="hidden" id="epf-fam-existing-id"/>
    <div style="display:flex;gap:16px;margin-bottom:12px">
      <label style="display:flex;align-items:center;gap:7px;cursor:pointer;font-size:.78rem;font-weight:600">
        <input type="radio" name="epf-mode" value="existing" checked onchange="toggleEpfMode('existing')" style="accent-color:var(--navy);width:15px;height:15px"/>Đã có tài khoản
      </label>
      <label style="display:flex;align-items:center;gap:7px;cursor:pointer;font-size:.78rem;font-weight:600">
        <input type="radio" name="epf-mode" value="new" onchange="toggleEpfMode('new')" style="accent-color:var(--navy);width:15px;height:15px"/>Tạo tài khoản mới
      </label>
    </div>
    <div class="field">
      <label>Họ tên người nhà *</label>
      <div class="ac-wrap">
        <input id="epf-fam-name" type="text" placeholder="Nhập tên để tìm..." autocomplete="off"
          oninput="acSearchEpf(this.value)" onfocus="acSearchEpf(this.value)"/>
        <div class="ac-dropdown" id="epf-ac-drop"></div>
      </div>
    </div>
    <div id="epf-new-fields" style="display:none">
      <div class="field-row c2">
        <div class="field"><label>SĐT</label><input id="epf-phone" type="text" maxlength="10" oninput="validatePhone(this)" onkeypress="return /[0-9]/.test(event.key)"/></div>
        <div class="field"><label>Email</label><input id="epf-email" type="email"/></div>
      </div>
      <div class="field"><label>Mật khẩu app</label><input id="epf-pw" type="text" value="123456"/></div>
    </div>
    ...
    <div style="margin:8px 0">
      <label style="display:flex;align-items:center;gap:8px;font-size:.76rem;font-weight:600;cursor:pointer">
        <input type="checkbox" id="epf-primary" style="accent-color:var(--navy);width:14px;height:14px"/>
        Là người giám sát chính
      </label>
    </div>
    <div class="modal-msg" id="epf-msg"></div>
    <div class="modal-footer">
      <button class="btn-cancel" onclick="closeModal('modal-edit-pt-family')">Hủy</button>
      <button class="btn-save" id="epf-save-btn" onclick="saveEditPtFamily()">💾 Lưu thay đổi</button>
    </div>
  </div>
</div>
...
function openEditPtFamily(linkId, userId, name, phone, email, relation, isPrimary){
  // Lưu linkId hiện tại để xóa sau
  document.getElementById('epf-old-link-id').value = linkId;
  document.getElementById('epf-patient-id').value  = ''; // sẽ lấy từ context
  ...
  const openCard = document.querySelector('.page-card.open');
  if(openCard){
    const pid = openCard.id.replace('pt-card-','');
    document.getElementById('epf-patient-id').value = pid;
  }

  // Autocomplete người nhà — dùng familiesCache
  const nameInput = document.getElementById('epf-fam-name');
  nameInput.value = name;
  document.getElementById('epf-fam-existing-id').value = userId;
  document.getElementById('epf-new-fields').style.display = 'none';

  // Load radio về "Đã có tài khoản"
...
function toggleEpfMode(mode){
  ...
  document.getElementById('epf-fam-name').value = '';
  document.getElementById('epf-fam-existing-id').value = '';
  document.getElementById('epf-ac-drop').style.display = 'none';
}

let _acEpfTimer = null;
function acSearchEpf(q){
  clearTimeout(_acEpfTimer);
  const drop = document.getElementById('epf-ac-drop');
  const mode = document.querySelector('input[name="epf-mode"]:checked')?.value;
  if(mode === 'new' || !q.trim()){ drop.style.display='none'; return; }
  _acEpfTimer = setTimeout(function(){
    const lq = q.toLowerCase();
    const matches = familiesCache.filter(function(f){
      return (f.userName||'').toLowerCase().includes(lq);
    }).slice(0,6);
    if(!matches.length){ drop.style.display='none'; return; }
    drop.innerHTML = matches.map(function(f){
      return '<div class="ac-item" onmousedown="acSelectEpf(\''+f.userId+'\',\''+esc(f.userName)+'\')">'+esc(f.userName)+(f.phone?' · '+f.phone:'')+'</div>';
    }).join('');
    drop.style.display='block';
  }, 200);
}

function acSelectEpf(userId, name){
  document.getElementById('epf-fam-name').value = name;
  document.getElementById('epf-fam-existing-id').value = userId;
  document.getElementById('epf-ac-drop').style.display = 'none';
}
...
async function saveEditPtFamily(){
  ...
  const existingId = document.getElementById('epf-fam-existing-id').value;
  const famName   = document.getElementById('epf-fam-name').value.trim();
  const mode      = document.querySelector('input[name="epf-mode"]:checked')?.value;
